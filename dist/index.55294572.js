const e=document.querySelector(".cartQty"),t=document.getElementById("catalogoSection");function o(){let t=JSON.parse(localStorage.getItem("cart"));t?e.innerText=t.reduce((e,t)=>e+t.qty,0):e.innerText=0}o();class r{constructor(e,t,o,r,n){this.id=e,this.nombre=t,this.imagen=o,this.precio=r,this.precio_viejo=n}}let n=[{id:"cb1",nombre:"Claus Bag 1",imagen:"public/products/cbRed_xs.webp",precio:"55000",precio_viejo:"70,000"},{id:"cb2",nombre:"Claus Bag 2",imagen:"public/products/cbGreen_xs.webp",precio:"55000",precio_viejo:"70,000"},{id:"cb3",nombre:"Claus Bag 3",imagen:"public/products/cb_3_xs.webp",precio:"50000",precio_viejo:"65,000"},{id:"cb4",nombre:"Claus Bag 4",imagen:"public/products/HK-X8_xs.webp",precio:"50000",precio_viejo:"65,000"},{id:"cb5",nombre:"Claus Bag 5",imagen:"public/products/HK-32_xs.webp",precio:"60000",precio_viejo:"75,000"},{id:"cb6",nombre:"Claus Bag 6",imagen:"public/products/CB-LineaVerde_xs.webp",precio:"60000",precio_viejo:"75,000"},{id:"cb7",nombre:"Claus Bag 7",imagen:"public/products/CB-LineaRoja_xs.webp",precio:"60000",precio_viejo:"75,000"},{id:"cb8",nombre:"Claus Bag 8",imagen:"public/products/HK-19_xs.webp",precio:"50000",precio_viejo:"65,000"}];const a={};function i(e){let t=e.target.closest(".productContainer"),o=t.querySelector("[data-pid]").dataset.pid,r=t.querySelector(".item-count");if(!t){console.error("Product Container Not Found");return}if(!o){console.error("Product ID not found");return}a[o]||(a[o]=1),"+"==e.target.textContent.trim()?(a[o]++,r.innerText=a[o]):"-"==e.target.textContent.trim()?a[o]>1&&(a[o]--,r.innerText=a[o]):console.log("doesn't should pass for here")}let l=JSON.parse(localStorage.getItem("cart"))?JSON.parse(localStorage.getItem("cart")):[];function c(e){let t=e.target.closest(".productContainer").querySelector(".item-count"),n=e.target.dataset.pid,i=l.findIndex(e=>e.id==n);if(-1==i){let o=new r(e.target.dataset.pid,e.target.dataset.name,e.target.dataset.img,e.target.dataset.price);o.qty=parseInt(t.textContent),l.push(o)}else l[i].qty+=parseInt(t.textContent);localStorage.setItem("cart",JSON.stringify(l)),o(),a[n]=1,Swal.mixin({toast:!0,position:"top-end",showConfirmButton:!1,timer:2e3,timerProgressBar:!0,didOpen:e=>{e.addEventListener("mouseenter",Swal.stopTimer),e.addEventListener("mouseleave",Swal.resumeTimer)}}).fire({icon:"success",text:`A\xf1adiste ${parseInt(t.textContent)} ClausBag al carrito `}),t.innerText=a[n],fbq("track","AddToCart",{model:n,qty:a[n]})}function s(){return l.reduce((e,t)=>e+=t.qty,0)}function d(){let e=document.querySelector(".totalCart"),t=l.reduce((e,t)=>(console.log({precio:t.precio}),console.log({total:e+=parseInt(t.qty)*parseInt(t.precio)}),e),0),o=`${t.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",")}`;return console.log(t),e.innerHTML=`Total: <span class='bold dark:text-green-400'>$${o}</span>`,t}function u(){l=JSON.parse(localStorage.getItem("cart"))?JSON.parse(localStorage.getItem("cart")):[],document.querySelector(".modalCartItemsContainer").innerHTML="";let e=document.querySelector(".buyBtnContainer");if(e.innerHTML="",0==l.length){document.querySelector(".purchaseQty").innerHTML=`
    ...
    <div data-pid=${e.id} class="sm:w-40">
      <figure class="flex align-top w-28 h-28 sm:h-44  pb-0 rounded-lg m-1 sm:w-44">
        ...
        Eliminar
            ...
            </div>`,document.querySelector(".modalCartItemsContainer").appendChild(t),document.querySelectorAll(".btn-cartCount").forEach(e=>e.addEventListener("click",m)),document.querySelectorAll(".deleteProductCart").forEach(e=>e.addEventListener("click",p)),document.querySelector(".purchaseQty").innerText=`Tienes ${s()} ClausBags en tu Carrito`}),document.querySelector(".buyBtnContainer").innerHTML=`
...
</button>`,e.classList.remove("hidden"),document.getElementById("confirmCartBtn").addEventListener("click",g),d(),function(){let e=document.getElementById("discountMsg");s()>=3?e.innerHTML=`<p class="text-2xl sm:text-2xl text-green-400 lg:text-3xl">Llevas 3 o m\xe1s. T\xfa env\xedo es gratis!</p>`:e.innerHTML=""}();let t=document.getElementById("modalCartContainer");t.classList.remove("hidden"),document.getElementById("closeModalCart").addEventListener("click",()=>t.classList.add("hidden"))}function m(e){let t=e.target.closest(".productContainer"),r=t.querySelector(".item-count"),n=t.querySelector("[data-pid]").dataset.pid,a=l.findIndex(e=>e.id==n);"+"==e.target.textContent.trim()?(l[a].qty+=1,r.innerText=l[a].qty):"-"==e.target.textContent.trim()?l[a].qty>1?(l[a].qty-=1,r.innerText=l[a].qty):1==l[a].qty&&p(e):console.log("doesn't should pass for here"),localStorage.setItem("cart",JSON.stringify(l)),s(),u(),o()}function g(){let e=l.sort((e,t)=>{let o=e.nombre.toLowerCase(),r=t.nombre.toLowerCase();return o<r?-1:o>r?1:0}).map(e=>`Modelo: *${e.nombre.replace(/_/g," ")}.* Precio: *${e.precio}* x Cantidad: *${e.qty}* = *${e.precio*e.qty}*`).join("\n"),t=`Hola, Quisiera Comprar estas ClausBags \u{1F381}: 
${e}
*Total Pedido = ${d()}*`,o=`https://wa.me/573202806790?text=${encodeURIComponent(t)}`;fbq("track","Purchase",{amount:d(),cartQty:s()}),window.open(o,"_blank")}function p(e){let t=e.target.dataset.pid,r=l.filter(e=>e.id!=t);localStorage.setItem("cart",JSON.stringify(r)),s(),u(),o(),d()}document.getElementById("cartList").addEventListener("click",u);const b=document.getElementById("navBar_mobileMenu"),x=document.getElementById("hamburger-btn"),f=document.getElementById("closeModalBtn");function y(){b.style.opacity=0,setTimeout(()=>{b.style.display="none"},500)}window.onclick=function(e){e.target===b&&(b.style.opacity=0,setTimeout(()=>{b.style.display="none"},500))},x.addEventListener("click",function(){b.style.display="block",setTimeout(()=>{b.style.opacity=1},10)}),f.addEventListener("click",y);const h=document.querySelector(".goToCart"),v=document.querySelector(".goToCartFromMobile"),w=document.getElementById("buyNowBtn");function C(){if(0==l.length){t.scrollIntoView({behavior:"smooth"}),y();return}u(),y()}h.addEventListener("click",C),v.addEventListener("click",C),w.addEventListener("click",C),window.addEventListener("scroll",()=>{let e=window.scrollY,t=document.querySelector(".wtspIconContainer");e>=1e3?t.classList.add("show"):t.classList.remove("show")}),new IntersectionObserver((e,t)=>{e.forEach(e=>{e.isIntersecting&&(console.log("showing products"),n.forEach(e=>{let t=document.createElement("div"),o=`$${e.precio.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",")}`;t.setAttribute("class","productContainer w-full shadow-xl flex flex-row p-1 sm:p-4 dark:bg-[#1E3D32] bg-white rounded-lg border border-gray-300  md:w-[48%] lg:w-[49%]"),t.innerHTML=`
    ...
    `,document.querySelector(".catalogoProductsContainer").appendChild(t)}),document.querySelectorAll(".addToCartBtn").forEach(e=>{e.addEventListener("click",c)}),document.querySelectorAll(".btn-count").forEach(e=>{e.addEventListener("click",i)}),t.unobserve(e.target))})},{root:null,rootMargin:"0px",threshold:.05}).observe(t);
//# sourceMappingURL=index.55294572.js.map
