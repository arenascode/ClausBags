const e=document.querySelector(".cartQty"),t=document.getElementById("catalogoSection");function o(){let t=JSON.parse(localStorage.getItem("cart"));t?e.innerText=t.reduce((e,t)=>e+t.qty,0):e.innerText=0}o();class n{constructor(e,t,o,n){this.id=e,this.nombre=t,this.imagen=o,this.precio=n}}console.log("show products Function"),[{id:"cb1",nombre:"Claus Bag 1",imagen:"public/products/HK-06_xs.webp",precio:"55000"},{id:"cb2",nombre:"Claus Bag 2",imagen:"public/products/HK-08_xs.webp",precio:"55000"},{id:"cb3",nombre:"Claus Bag 3",imagen:"public/products/cb_3_xs.webp",precio:"55000"},{id:"cb4",nombre:"Claus Bag 4",imagen:"public/products/HK-X8_xs.webp",precio:"55000"},{id:"cb5",nombre:"Claus Bag 5",imagen:"public/products/HK-32_xs.webp",precio:"60000"},{id:"cb6",nombre:"Claus Bag 6",imagen:"public/products/CB-LineaVerde_xs.webp",precio:"60000"},{id:"cb7",nombre:"Claus Bag 7",imagen:"public/products/CB-LineaRoja_xs.webp",precio:"60000"},{id:"cb8",nombre:"Claus Bag 8",imagen:"public/products/HK-19_xs.webp",precio:"55000"}].forEach(e=>{let t=document.createElement("div");t.setAttribute("class","productContainer w-full shadow-xl flex flex-row p-1 sm:p-4 dark:bg-[#1E3D32] bg-white rounded-lg border border-gray-300  md:w-[48%] lg:w-[49%]"),t.innerHTML=`
    ...
    `,document.querySelector(".catalogoProductsContainer").appendChild(t)}),document.querySelectorAll(".addToCartBtn").forEach(e=>{e.addEventListener("click",s)}),document.querySelectorAll(".btn-count").forEach(e=>{e.addEventListener("click",a)});const r={};function a(e){let t=e.target.closest(".productContainer"),o=t.querySelector("[data-pid]").dataset.pid;console.log(o);let n=t.querySelector(".item-count");if(!t){console.error("Product Container Not Found");return}if(!o){console.error("Product ID not found");return}r[o]||(r[o]=1,console.log(r)),"+"==e.target.textContent.trim()?(r[o]++,n.innerText=r[o]):"-"==e.target.textContent.trim()?r[o]>1&&(r[o]--,n.innerText=r[o]):console.log("doesn't should pass for here")}let l=JSON.parse(localStorage.getItem("cart"))?JSON.parse(localStorage.getItem("cart")):[];function s(e){let t=e.target.closest(".productContainer").querySelector(".item-count"),a=e.target.dataset.pid,s=l.findIndex(e=>e.id==a);if(console.log(s),-1==s){let o=new n(e.target.dataset.pid,e.target.dataset.name,e.target.dataset.img,e.target.dataset.price);o.qty=parseInt(t.textContent),l.push(o)}else l[s].qty+=parseInt(t.textContent);localStorage.setItem("cart",JSON.stringify(l)),o(),r[a]=1,Swal.mixin({toast:!0,position:"top-end",showConfirmButton:!1,timer:2e3,timerProgressBar:!0,didOpen:e=>{e.addEventListener("mouseenter",Swal.stopTimer),e.addEventListener("mouseleave",Swal.resumeTimer)}}).fire({icon:"success",text:`A\xf1adiste ${parseInt(t.textContent)} ClausBag al carrito `}),t.innerText=r[a],fbq("track","AddToCart",{model:a,qty:r[a]})}function i(){return l.reduce((e,t)=>e+=t.qty,0)}function c(){let e=document.querySelector(".totalCart"),t=l.reduce((e,t)=>e+=parseInt(t.qty)*parseInt(t.precio),0);return console.log(t),e.innerHTML=`Total: <span class='bold dark:text-green-400'>$${t}</span>`,t}function d(){l=JSON.parse(localStorage.getItem("cart"))?JSON.parse(localStorage.getItem("cart")):[],console.log("testing button"),document.querySelector(".modalCartItemsContainer").innerHTML="";let e=document.querySelector(".buyBtnContainer");if(e.innerHTML="",0==l.length){document.querySelector(".purchaseQty").innerHTML=`
    ...
    <div data-pid=${e.id} class="sm:w-40">
      <figure class="flex align-top w-28 h-28 sm:h-44  pb-0 rounded-lg m-1 sm:w-44">
        ...
        Eliminar
            ...
            </div>`,document.querySelector(".modalCartItemsContainer").appendChild(t),document.querySelectorAll(".btn-cartCount").forEach(e=>e.addEventListener("click",u)),document.querySelectorAll(".deleteProductCart").forEach(e=>e.addEventListener("click",g)),document.querySelector(".purchaseQty").innerText=`Tienes ${i()} ClausBags en tu Carrito`}),document.querySelector(".buyBtnContainer").innerHTML=`
...
</button>`,e.classList.remove("hidden"),document.getElementById("confirmCartBtn").addEventListener("click",m),c(),function(){console.log("calculating disccount");let e=document.getElementById("discountMsg");console.log(i),i()>=3?e.innerHTML=`<p class="text-xl sm:text-xl text-green-400 lg:text-3xl">Llevas 3 o m\xe1s. Te haremos el descuento en Whatsapp</p>`:e.innerHTML=""}();let t=document.getElementById("modalCartContainer");t.classList.remove("hidden"),document.getElementById("closeModalCart").addEventListener("click",()=>t.classList.add("hidden"))}function u(e){let t=e.target.closest(".productContainer"),n=t.querySelector(".item-count"),r=t.querySelector("[data-pid]").dataset.pid;console.log(r);let a=l.findIndex(e=>e.id==r);"+"==e.target.textContent.trim()?(l[a].qty+=1,n.innerText=l[a].qty):"-"==e.target.textContent.trim()?(console.log(l[a].qty),l[a].qty>1?(l[a].qty-=1,n.innerText=l[a].qty,console.log("did enter in qty > 1?")):1==l[a].qty&&(console.log("did enter in qty == 1?"),g(e))):console.log("doesn't should pass for here"),localStorage.setItem("cart",JSON.stringify(l)),i(),d(),o()}function m(){console.log(l);let e=l.sort((e,t)=>{let o=e.nombre.toLowerCase(),n=t.nombre.toLowerCase();return o<n?-1:o>n?1:0}).map(e=>`Modelo: ${e.nombre.replace(/_/g," ")} - Cantidad: ${e.qty}`).join("\n"),t=`Hola, Quisiera Comprar estas ClausBags \u{1F381}: 
${e}`,o=`https://wa.me/573202806790?text=${encodeURIComponent(t)}`;fbq("track","Purchase",{amount:c(),cartQty:i()}),window.open(o,"_blank")}function g(e){let t=e.target.dataset.pid;console.log(t),console.log(l);let n=l.filter(e=>e.id!=t);console.log(n),localStorage.setItem("cart",JSON.stringify(n)),i(),d(),o(),c()}console.log({cart:l}),console.log(!!localStorage.getItem("cart")),document.getElementById("cartList").addEventListener("click",d);const p=document.getElementById("navBar_mobileMenu"),b=document.getElementById("hamburger-btn"),x=document.getElementById("closeModalBtn");function f(){console.log("hiii"),p.style.opacity=0,setTimeout(()=>{p.style.display="none"},500)}window.onclick=function(e){e.target===p&&(p.style.opacity=0,setTimeout(()=>{p.style.display="none"},500))},b.addEventListener("click",function(){p.style.display="block",setTimeout(()=>{p.style.opacity=1},10)}),x.addEventListener("click",f);const y=document.querySelector(".goToCart"),h=document.querySelector(".goToCartFromMobile"),w=document.getElementById("buyNowBtn");function C(){if(console.log("hii"),console.log({cart:l}),0==l.length){t.scrollIntoView({behavior:"smooth"}),f();return}d(),f()}y.addEventListener("click",C),h.addEventListener("click",C),w.addEventListener("click",C),window.addEventListener("scroll",()=>{let e=window.scrollY,t=document.querySelector(".wtspIconContainer");e>=1e3?t.classList.add("show"):t.classList.remove("show")});
//# sourceMappingURL=index.adfaaed4.js.map
