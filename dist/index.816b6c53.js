    ...
    <div data-pid=${e.id} class="sm:w-40">
      <figure class="flex align-top w-28 h-28 sm:h-44  pb-0 rounded-lg m-1 sm:w-44">
        ...
        Eliminar
            ...
            </div>`,document.querySelector(".modalCartItemsContainer").appendChild(t),document.querySelectorAll(".btn-cartCount").forEach(e=>e.addEventListener("click",u)),document.querySelectorAll(".deleteProductCart").forEach(e=>e.addEventListener("click",m)),document.querySelector(".purchaseQty").innerText=`Tienes ${s()} ClausBags en tu Carrito`}),document.querySelector(".buyBtnContainer").innerHTML=`
...
</button>`,e.classList.remove("hidden"),document.getElementById("confirmCartBtn").addEventListener("click",g),c(),function(){console.log("calculating disccount");let e=document.getElementById("discountMsg");console.log(s),s()>=3?e.innerHTML=`<p class="text-xl sm:text-xl text-green-400 lg:text-3xl">Llevas 3 o m\xe1s. Te haremos el descuento en Whatsapp</p>`:e.innerHTML=""}();let t=document.getElementById("modalCartContainer");t.classList.remove("hidden"),document.getElementById("closeModalCart").addEventListener("click",()=>t.classList.add("hidden"))}function u(e){let t=e.target.closest(".productContainer"),n=t.querySelector(".item-count"),r=t.querySelector("[data-pid]").dataset.pid;console.log(r);let a=l.findIndex(e=>e.id==r);"+"==e.target.textContent.trim()?(l[a].qty+=1,n.innerText=l[a].qty):"-"==e.target.textContent.trim()?(console.log(l[a].qty),l[a].qty>1?(l[a].qty-=1,n.innerText=l[a].qty,console.log("did enter in qty > 1?")):1==l[a].qty&&(console.log("did enter in qty == 1?"),m(e))):console.log("doesn't should pass for here"),localStorage.setItem("cart",JSON.stringify(l)),s(),d(),o()}function g(){console.log(l);let e=l.sort((e,t)=>{let o=e.nombre.toLowerCase(),n=t.nombre.toLowerCase();return o<n?-1:o>n?1:0}).map(e=>`Modelo: ${e.nombre.replace(/_/g," ")} - Cantidad: ${e.qty}`).join("\n"),t=`Hola, Quisiera Comprar estas ClausBags \u{1F381}: 
${e}`,o=`https://wa.me/573209389966?text=${encodeURIComponent(t)}`;window.open(o,"_blank")}function m(e){let t=e.target.dataset.pid;console.log(t),console.log(l);let n=l.filter(e=>e.id!=t);console.log(n),localStorage.setItem("cart",JSON.stringify(n)),s(),d(),o(),c()}console.log({cart:l}),console.log(!!localStorage.getItem("cart")),document.getElementById("cartList").addEventListener("click",d);const p=document.getElementById("navBar_mobileMenu"),b=document.getElementById("hamburger-btn"),x=document.getElementById("closeModalBtn");function f(){console.log("hiii"),p.style.opacity=0,setTimeout(()=>{p.style.display="none"},500)}window.onclick=function(e){e.target===p&&(p.style.opacity=0,setTimeout(()=>{p.style.display="none"},500))},b.addEventListener("click",function(){p.style.display="block",setTimeout(()=>{p.style.opacity=1},10)}),x.addEventListener("click",f);const y=document.querySelector(".goToCart"),h=document.querySelector(".goToCartFromMobile"),w=document.getElementById("buyNowBtn");function C(){if(console.log("hii"),console.log({cart:l}),0==l.length){t.scrollIntoView({behavior:"smooth"}),f();return}d(),f()}y.addEventListener("click",C),h.addEventListener("click",C),w.addEventListener("click",C);
//# sourceMappingURL=index.816b6c53.js.map
